...
resource "twingate_resource" "additional_resources" {
  ...
  group_ids         = concat(
    ...
    # [for group in local.resource_group_existing: data.twingate_groups.additional_resources_existing_groups["${each.value.name}-${group.name}"].groups.id if can(data.twingate_groups.additional_resources_existing_groups["${each.value.name}-${group.name}"])]
