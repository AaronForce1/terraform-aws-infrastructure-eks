module "eks_managed_node_group" {
  ...
  for_each = {
    for node_group in var.eks_managed_node_groups : node_group.name =>
    node_group
  }
  ...
  iam_role_name              = "${module.eks.cluster_name}-${each.value.name}"
  iam_role_attach_cni_policy = true
  iam_role_use_name_prefix   = false
  iam_role_tags              = local.base_tags

  launch_template_name            = "${module.eks.cluster_name}-${each.value.name}"
