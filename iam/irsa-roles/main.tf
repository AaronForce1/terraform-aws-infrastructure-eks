module "iam-iam-role-for-service-accounts-eks" {
  ...
  role_name = "${var.app_name}-${var.app_namespace}-${var.tfenv}-${try(each.value.name, each.key)}"
  ...
  cert_manager_hosted_zone_arns = lookup(var.route53_hosted_zone_arns, "route53_hosted_zone_arns", ["arn:aws:route53:::hostedzone/*"])
  ...
  external_dns_hosted_zone_arns = lookup(var.route53_hosted_zone_arns, "route53_hosted_zone_arns", ["arn:aws:route53:::hostedzone/*"])

  # external-secrets
  attach_external_secrets_policy        = each.key == "external-secrets-policy" ? true : false
  external_secrets_kms_key_arns         = try(each.value.external_secrets_kms_key_arns, ["arn:aws:kms:::key/*"])
  external_secrets_secrets_manager_arns = try(each.value.external_secrets_secrets_manager_arns, ["arn:aws:secretsmanager:::secret:*"])
  external_secrets_ssm_parameter_arns   = try(each.value.external_secrets_ssm_parameter_arns, ["arn:aws:ssm:::parameter/*"])
  ...
  oidc_providers = {
    main = {
      ...
      namespace_service_accounts = try(each.value.attach_efs_csi_policy, false) == true ? ["${each.value.namespace}:${each.value.service_account_ebs}", "${each.value.namespace}:${each.value.service_account_efs}"] : ["${each.value.namespace}:${each.value.service_account}"]
    }
  }
}
